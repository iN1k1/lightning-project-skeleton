...
def reconstruction_batch(x, model, size:int=320):
    ...
    imgs = [custom_to_pil(x) for x in x_rec]
    return imgs
...
def run(opt):
    ...
    model = load_vcgan(config, ckpt_path=opt.checkpoint).to(device)
    ...
    # for img_paths in tqdm(image_paths_chunks, desc=colored('Reconstructing', print_color)):
    for iteration_test, (imgs, img_paths) in tqdm(enumerate(testing_data_loader)):
        imgs = reconstruction_batch(imgs.to(model.device), model)

        for ii, img in enumerate(imgs):
            img_path = Path(img_paths[ii])
            img.save(str(output_folder / img_path.name))

            # Save as npy for numerical comparisons
            # np.save(output_folder / f"{img_path.stem}.npy", np.array(img))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--size', type=int, default=-1, help='Image size (-1 = keep original size)')
